    ...
    async (req, res) => {
        ...
        `,
            ...
            if (duplicateCodeResult.rows.length > 0) {
                return res.status(400).json({
                    message:
                        "Kode barang tersebut sudah digunakan oleh produk lain"
                });
            }
...
app.get(
    ...
    async (req, res) => {
        try {
            ...
            res.status(500).json({
                message: "Gagal mengambil riwayat barang masuk",
                error: error.message
            });
        }
    }
...
// ======================================================
// UPDATE CATATAN OUTBOUND
// ADMIN UTAMA, ADMIN 2, ADMIN 3
// ======================================================

app.patch(
    "/api/outbound-orders/:id/notes",
    authenticateToken,
    authorizeRoles("admin_utama", "admin_2", "admin_3"),
    async (req, res) => {
        ...
        try {
            ...
            const result = await pool.query(
                ...
                SET
                    ...
                    updated_at = CURRENT_TIMESTAMP
                WHERE id = $2
                RETURNING id, invoice_number, notes, updated_at;
                `,
                [
                    notes?.trim() || null,
                    orderId
...
// ======================================================
// RIWAYAT MATERIAL ISSUE
// ======================================================

app.get(
    "/api/material-issues/history",
    authenticateToken,
    authorizeRoles(
        "admin_utama",
        "admin_2",
        ...
        try {
            ...
            const result = await pool.query(`
                ...
                JOIN material_issue_items mii
                    ON mii.material_issue_id = mi.id
            ...
            res.status(500).json({
                message: "Gagal mengambil riwayat material issue",
                error: error.message
            });
...
app.post(
    "/api/material-issues",
    authenticateToken,
    authorizeRoles("admin_utama", "gudang"),
    async (req, res) => {
        const {
            issue_number,
            team_name,
...
app.get(
    ...
    async (req, res) => {
        ...
        try {
            ...
            let whereClause =
                "WHERE st.direction = 'IN' AND st.transaction_type = 'PURCHASE'";
            const params = [];

            if (date_from) {
                params.push(date_from);
                whereClause += ` AND st.transaction_date >= $${params.length}`;
            }
...
// ======================================================
// GET LAPORAN MATERIAL ISSUE
// ======================================================

app.get(
    "/api/reports/material-issues",
    authenticateToken,
    authorizeRoles("admin_utama", "admin_2", "admin_3", "gudang"),
    async (req, res) => {
        ...
        try {
            ...
            if (date_from) {
                params.push(date_from);
                whereClause += ` AND mi.issue_date >= $${params.length}`;
            }
            ...
            const result = await pool.query(
                ...
                LEFT JOIN units un            ON un.id  = p.unit_id
                LEFT JOIN users usr           ON usr.id = mi.issued_by
                ${whereClause}
                ORDER BY mi.issue_date DESC, mi.id DESC, mii.id ASC;
                `,
                params
            );
            ...
            res.status(500).json({
                message: "Gagal mengambil laporan material issue",
                error: error.message
            });
        }
    }
...
// ======================================================
// GET LAPORAN OUTBOUND
// ======================================================

app.get(
    "/api/reports/outbound",
    authenticateToken,
    authorizeRoles("admin_utama", "admin_2", "admin_3", "gudang"),
    async (req, res) => {
        ...
        try {
            ...
            let whereClause = "WHERE oo.status = 'SHIPPED'";
            const params = [];

            if (date_from) {
                params.push(date_from);
                whereClause += ` AND oo.transaction_date >= $${params.length}`;
            }
            ...
            const result = await pool.query(
                ...
                SELECT
                    ...
                    oo.destination,
                    oo.status,
                    oo.notes         AS doc_notes,
                    p.code,
                    p.name,
                    un.name          AS unit,
                    un.symbol        AS unit_symbol,
                    ooi.quantity,
                    ooi.notes        AS item_notes,
                    usr.name         AS created_by
                FROM outbound_orders oo
                JOIN outbound_order_items ooi ON ooi.outbound_order_id = oo.id
                JOIN products p               ON p.id   = ooi.product_id
                LEFT JOIN units un            ON un.id  = p.unit_id
                LEFT JOIN users usr           ON usr.id = oo.created_by
                ${whereClause}
                ORDER BY oo.transaction_date DESC, oo.id DESC, ooi.id ASC;
                `,
                params
            );
            ...
            res.status(500).json({
                message: "Gagal mengambil laporan outbound",
                error: error.message
            });
        }
    }
